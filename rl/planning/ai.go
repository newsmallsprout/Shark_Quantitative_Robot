...
const qwenPrompt = `你是加密货币主流币中长线交易分析师。判断 %s 的波段方向。
...
规则: 主流币中长线重仓，不做激进追单。杠杆20-35x，仓位8-18%%；止损给足ATR呼吸空间，至少3倍ATR，止盈看2-5倍ATR波段。confidence必须是0-100百分制数字，禁止用0-1小数或布尔0/1。long_entry_low < long_entry_high必须成立。只输出JSON，不要其他文字。`

const deepseekPrompt = `你是顶级加密货币主流币中长线交易员。为 %s 生成完整交易计划。
...
## 要求（主流币中长线重仓纪律 — 违反任何一条=废单）
- 杠杆20-35x（偏进攻但不能用超短线80x以上杠杆）别超出交易所对币对的上限
- 仓位8-18%%（position_size_pct用小数 0.12=12%%，主流币重仓，别给蚊子仓）
- ⚠️ 首个止盈至少2倍ATR，第二止盈3.5-5倍ATR，趋势没坏就拿住
- ⚠️ 止损必须足够宽，至少 ≥ ATR14/价格×100×3.0，避免震荡洗掉
