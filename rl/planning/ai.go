...
func (p *Planner) callDeepSeek(ctx context.Context, pc *PlanContext, qwenHint *AIPlan) *AIPlan {
	...
	prompt := fmt.Sprintf(deepseekPrompt, pc.Symbol, pc.Price, pc.ATR14, pc.FundingRate,
		...
		pc.SupportStr, pc.ResistanceStr,
...
规则: 超短线(5-15分钟持仓)，区间收紧。只输出JSON，不要其他文字。`
...
## 要求
- 超短线（持仓5-15分钟），区间紧凑，快进快出
- 杠杆50-125x起步（高杠杆超短线标配），仓位1-5%%（position_size_pct用小数 0.02=2%%）
- 止损0.3-1.5%%价格幅度（高杠杆下止损必须紧）
- 止盈1-3%%价格幅度（快进快出吃小波段）
- pyramid_prices给1-2个补仓点，cut_loss_pct给绝对割肉线%%（用小数 0.05=5%%）
- rationale用中文（20-40字），针对该币对当前行情做具体分析，不要泛泛而谈
