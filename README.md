...
| `SHARK_API_TOKEN` | 必填 | REST/WS鉴权Bearer Token |
...
| POST | `/api/paper/toggle` | Bearer | 切换模拟盘开关 |
| GET | `/api/live/status` | Bearer | 实盘状态 |
| POST | `/api/live/toggle` | Bearer | 切换实盘开关 |
| POST | `/api/shark/mode` | Bearer | 切换 paper/live 模式 |
| GET | `/api/evo/pending` | Bearer | 待审批进化建议 |
| POST | `/api/evo/approve/{id}` | Bearer | 审批通过 |
| POST | `/api/evo/reject/{id}` | Bearer | 拒绝 |
